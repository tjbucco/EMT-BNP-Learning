function [daoed_est, object_class_est_update, state_est_update] = detect_objects_and_associate_data_FUSION(object_class_est, state_est, daoed_est_prev, measurements, data, hyper, script, knownparams_flag, t)
...
if no_of_legacyobjects == 0
    ...
    if knownparams_flag
        ...
        [D, Q, C_new] = assignclassCRPheuristic_FUSION(no_of_objects_update, object_class_est.Dest(:,:,~isnan(object_class_est.Dest(1,1,:))), object_class_est.Qest(:,:,~isnan(object_class_est.Qest(1,1,:))), object_class_est.Cest, hyper);
        if isempty(object_class_est.Cest)
            ...
            object_class_est_update.Dest = cat(3, object_class_est_update.Dest(:,:,~isnan(object_class_est_update.Dest(1,1,:))), D(:,:, unique(C_new(C_new(newobject_exists > 0) > max(object_class_est.Cest)))));
            object_class_est_update.Qest = cat(3, object_class_est_update.Qest(:,:,~isnan(object_class_est_update.Qest(1,1,:))), Q(:,:, unique(C_new(C_new(newobject_exists > 0) > max(object_class_est.Cest)))));
        end
    end
    ...
    [clustered_meas, meas_to_cluster_labels, no_of_newobjects_em, clutterindex] = cluster_measurements_em_FUSION(measurements, hyper, script, sum(daoed_est_prev.exist_ind_all > 0), script.cov_comp.*Drot(:,:,daoed_est_prev.exist_ind_all > 0), X_predict(1:2, daoed_est_prev.exist_ind_all > 0));
    ...
    if no_of_newobjects_est > 0
        if knownparams_flag
            ...
            [D, Q, C_new] = assignclassCRPheuristic_FUSION(no_of_newobjects_est, object_class_est.Dest(:,:,~isnan(object_class_est.Dest(1,1,:))), object_class_est.Qest(:,:,~isnan(object_class_est.Qest(1,1,:))), object_class_est.Cest, hyper);
        end
        ...
        %[X_new_loc, meas_to_newbirth_labels, no_of_newobjects_em, clutterindex] = cluster_measurements_em_FUSION(measurements(meas_to_object_labels == 0), hyper, script, 1, Davg);
        ...
        if no_of_newobjects_est_round2 > 0
            ...
            if knownparams_flag
                ...
                object_class_est_update.Dest = cat(3, object_class_est_update.Dest(:,:,~isnan(object_class_est_update.Dest(1,1,:))), D(:,:, unique(C_new(C_new(newobject_exists) > max(object_class_est.Cest)))));
                object_class_est_update.Qest = cat(3, object_class_est_update.Qest(:,:,~isnan(object_class_est_update.Qest(1,1,:))), Q(:,:, unique(C_new(C_new(newobject_exists) > max(object_class_est.Cest)))));
            end
